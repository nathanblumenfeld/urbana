...
?>
  <div class="login-form">
    <form action="<?php echo htmlspecialchars($action) ?>" method="post" novalidate>
      ...
      <input class="login-input" id="username" type="text" name="login_username" placeholder="username" value="<?php echo htmlspecialchars($sticky_login_username); ?>" required />
      <label for="password"></label>
      <input class="login-input" id="password" type="password" name="login_password" placeholder="password" required />
      <button class="login-button" name="login" type="submit">Sign In</button>
    </form>
    <ul class="login-feedback">
      <?php
...
function find_saved_listings($db, $current_user_id) {
  $db->beginTransaction();
  $listings = exec_sql_query($db, "SELECT DISTINCT listings.id AS ID, saves.saved_by FROM listings LEFT JOIN saves ON saves.listing_id = listings.id WHERE (saves.saved_by = $current_user_id)",
    array()
  );
  $db->commit();
  $user_saved_listings = array();
  foreach ($listings as $listing) {
    array_push($user_saved_listings, $listing['ID']);
  }
  return $user_saved_listings;
}
